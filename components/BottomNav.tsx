...
export default function BottomNav({ activeTab, setActiveTab }: BottomNavProps) {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-white/95 backdrop-blur-md border-t border-slate-200/90 shadow-lg">
      <div className="max-w-md sm:max-w-xl md:max-w-3xl lg:max-w-5xl mx-auto px-2 sm:px-4 flex justify-around items-center h-15 sm:h-16">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              ...
              <span className="text-[10px] sm:text-xs mt-0.5 leading-none whitespace-nowrap truncate max-w-full">{item.label}</span>
            </button>
          );
        })}
      </div>
